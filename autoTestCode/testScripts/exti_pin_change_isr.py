...
def exti_pin_change_isr_test():
    ...
    stimulus = {"done": False, "start": time.monotonic()}

    def pulse_pd3():
        if (stimulus["done"]):
            return
        if (time.monotonic() - stimulus["start"] < 0.002):
            return
        test_ch32v003_test_target.digital_write(0, True)
        stimulus["done"] = True
    ...
    if (not stimulus["done"]):
        print("Stimulus pulse was not sent during capture")
        return False
    ...
    if (blips < 0):
        ...
        return False
    if (blips < 1):
        print(f"Expected at least 1 blip on PA7, found {blips}")
        return False

    print(f"Detected {blips} EXTI blip(s) on PA7")
    return True
